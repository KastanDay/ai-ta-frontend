	...
	if (isValidApiKey) {
		try {
			...
			const { error: updateError } = await supabase
				.rpc('increment', { usage: 1, apikey: apiKey });
...
export default async function handler(req: NextRequest, res: NextResponse) {
	try {
		...
		const { api_key, course_name } = await req.json() as { api_key: string, course_name: string };

		console.log('api_key', api_key, 'course_name', course_name);

		// Validate the API key and retrieve the user object.
		const { isValidApiKey, userObject } = await validateApiKeyAndRetrieveData(api_key, course_name);
		...
		return NextResponse.json({ error: 'An error occurred while validating the API key' }, { status: 500 });
	}
}
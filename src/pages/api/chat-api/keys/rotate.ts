...
export default async function rotateKey(req: NextRequest, res: NextResponse) {
	...
	if (existingKeyError) {
		...
		return NextResponse.json({ error: existingKeyError.message }, { status: 500 });
	}

	// If no existing key is found, inform the user to generate one.
	if (!existingKey || existingKey.length === 0) {
		return NextResponse.json({ error: 'API key not found for user, please generate one!' }, { status: 404 });
	}
	...
	return NextResponse.json({ message: 'API key rotated successfully', newApiKey }, { status: 200 });
}
 ...
 */
export default async function generateKey(req: NextRequest, res: NextResponse): Promise<NextResponse> {
	...
	try {
		...
		if (keys.length > 0 && keys[0]?.is_active) {
			return NextResponse.json({ error: 'User already has an API key' }, { status: 409 });
		}
		...
		return NextResponse.json({ message: 'API key generated successfully', apiKey }, { status: 200 });
	} catch (error) {

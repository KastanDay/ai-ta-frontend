...
export default async function chat(req: NextRequest): Promise<NextResponse> {
	...
	const { model, messages, openai_key, temperature, course_name, stream, api_key }: { model: string; messages: Message[]; openai_key: string; temperature: number; course_name: string; stream: boolean; api_key: string } = body;
	...
	const chatBody: ChatBody = constructChatBody(conversation, key, course_name, stream);

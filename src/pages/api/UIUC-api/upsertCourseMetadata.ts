...
import {type CourseMetadata, type CourseMetadataOptionalForUpsert,} from '~/types/courseMetadata'
...
export default async function handler(
  ...
  const { courseName, courseMetadata }: { courseName: string, courseMetadata: CourseMetadataOptionalForUpsert } = JSON.parse(requestBody);
  ...
  try {
    ...
    if (!courseMetadata.course_admins || courseMetadata.course_admins.length === 0) {

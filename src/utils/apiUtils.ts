...
export const callSetCourseMetadata = async (
  ...
  try {
    ...
    if (data.success) {
      log.debug('Course metadata updated successfully', { course_name: courseName, course_metadata: courseMetadata });
      return true;
    } else {
      log.error('Error setting course metadata', { course_name: courseName, error: data.error });
      return false;
    }
  } catch (error) {
    log.error('Error setting course metadata', { course_name: courseName, error });
    return false;
  }
};
 ...
 */
export const uploadToS3 = async (file: File | null, course_name: string): Promise<string | undefined> => {
  if (!file) return;
  ...
  const requestObject = {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ fileName: file.name, fileType: file.type, courseName: course_name, uniqueFileName }),
  };
